...
class OAuthUdbKeyMgr {
	private static $getUdbKeyURL = "https://udb.duowan.com/message/getudbkey?type=new";
	...
	private function __initUdbKeyCache(){
		...
		$to_update_udb_key = true;
		$to_update_secure_key = true;
		if($key){
			if(!empty($key->udb_key) && $key->udb_key->isLegal()){
				$this->udb_key_cache->udb_key = $key->udb_key;
				$to_update_udb_key = false;
			}
		...
		if($to_update_udb_key){
			$this->udb_key_cache->udb_key = $this->__getUdbKey();
		}
		...
		if (!empty($this->udb_key_cache->udb_key) && $this->udb_key_cache->udb_key->isLegal() &&
		!empty($this->udb_key_cache->secure_key) && $this->udb_key_cache->secure_key->isLegal()) {
	...
	private function __getUdbKey(){
		...
		curl_setopt($ch, CURLOPT_URL, self::$getUdbKeyURL);
		...
		$rt = explode(";",$response);
		if(count($rt) < 3){
			throw new OAuthException('get udb key failed!', 500);
		}
		$this->need_save = true;
		return new OAuthUdbKey($rt);;
	}
